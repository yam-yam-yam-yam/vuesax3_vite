<script setup>
import { ref } from 'vue'
</script>

<template>
    <vs-card actionable >
        <template v-slot:header>Hello world !</template>
        <template v-slot:media>
            <img src="data:image/png;base64,">
        </template>
        <template v-slot:extra-content style="font-size:11px !important;">extra-content</template>
        <template v-slot:footer>
            <vs-row vs-justify="flex-end">
                <vs-button type="gradient" color="danger" icon="favorite" style="margin-right:10px;"></vs-button>
                <vs-button color="primary" icon="turned_in_not" style="margin-right:10px;"></vs-button>
                <vs-button color="rgb(230,230,230)" color-text="rgb(50,50,50)" icon="settings"></vs-button>
            </vs-row>
        </template>
        <p>슬롯외부입니다.</p>
    </vs-card>

    <vs-card>
        <p>vs-card의 slot들엔 header,media,extra-content,footer가 있음</p>
        <p>header는 윗부분</p>
        <p>media는 내용</p>
        <p>extra-content는 내용부분의 아래부분</p>
        <p>footer는 아래부분</p>
        <p>card내부, slot외부에 텍스트를 입력시 media와 extra-content사이에 위치하게 된다.</p>
    </vs-card>
</template>

<style>

</style>